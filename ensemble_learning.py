...
def main(config):
    ...
    dataset_full.region_ids = np.loadtxt(
        "data/full_labeled_overlapped_regions_id.txt", dtype=object
    )
    labeled_loader = DataLoader(
        dataset=dataset_full, **config["data"]["dataloader"]["valid"], pin_memory=True
    )
    ...
    for i in range(num_models):
        ...
        print(
            "Score for head {} : {}".format(
                model.curr_head, f1_score(labels, predicted, average="weighted")
            )
        )
