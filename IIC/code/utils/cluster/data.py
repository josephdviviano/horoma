...
import torch
import torchvision
from .dataset import HoromaDataset
...
def cluster_twohead_create_dataloaders(config):
  ...
  elif "Horoma" in config.dataset:
    config.train_partitions_head_A = ["train_overlapped"] #variable split dans le notre classe dataset
    config.train_partitions_head_B = config.train_partitions_head_A

    config.mapping_assignment_partitions =["train_labeled_overlapped","valid_overlapped"]
    config.mapping_test_partitions = ["train_labeled_overlapped","valid_overlapped"]

    dataset_class = HoromaDataset
...
def cluster_create_dataloaders(config):
  ...
  if "CIFAR" in config.dataset:
    ...
    config.train_partitions= ["train_overlapped"] #variable split dans le notre classe dataset

    config.mapping_assignment_partitions = ["train_labeled_overlapped"]
    config.mapping_test_partitions = ["valid_overlapped"]
    #config.mapping_assignment_partitions = ["valid_overlapped"]
    #config.mapping_test_partitions = ["train_labeled_overlapped"]
    #config.mapping_assignment_partitions =["train_labeled_overlapped", "valid_overlapped"]
    #config.mapping_test_partitions = ["train_labeled_overlapped", "valid_overlapped"]
  ...
  dataloaders = \
    _create_dataloaders(config, dataset_class, tf1, tf2,
                        ...
                        target_transform=target_transform) #DATASET TO TRAIN THE ALGORITHM. SHOULD BE TRAIN_OVERLAPPED

  mapping_assignment_dataloader = \
    _create_mapping_loader(config, dataset_class, tf3,
                           partitions=config.mapping_assignment_partitions,
                           target_transform=target_transform) #DATASET USED TO ASSIGN A NUMBER TO A CLUSTER. SHOULD BE TRAIN_LABELED_OVERLAPPED

  mapping_test_dataloader = \
    _create_mapping_loader(config, dataset_class, tf3,
                           partitions=config.mapping_test_partitions,
                           target_transform=target_transform) #DATASET USED TO ASSIGN THE ACCURACY OF THE CLUSTERING. SHOULD BE VALID OVERLAPPED
  ...
  for d_i in range(config.num_dataloaders):
    ...
    for train_partition in partitions:
      if "STL10" == config.dataset:
        ...
        train_imgs_tf_curr = dataset_class(
          data_dir=config.dataset_root,
          transforms=tf2,  # random per call
          split=train_partition)
      else:
        train_imgs_tf_curr = dataset_class(
          root=config.dataset_root,
